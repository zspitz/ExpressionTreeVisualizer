...
namespace ExpressionToString.Tests {
    ...
    public class FormatStringParsing {
        private static void BuildAssert(string format, List<(string, int?, int?, string)> expected) =>
            Assert.Equal(expected, ParseFormatString(format));

        [Fact]
        void IndexOnly() => BuildAssert(
            "0 = {0} 1 = {1} 2 = {2} 3 = {3} 4 = {4}",
        ...
        [Fact]
        void EmptyStringItemFormat() => BuildAssert(
            "{0:}",
            new List<(string, int?, int?, string)> {
                ("", 0, null, "")
            }
